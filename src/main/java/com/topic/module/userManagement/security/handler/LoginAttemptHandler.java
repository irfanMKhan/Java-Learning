...
import com.topic.module.userManagement.utility.constant.ApplicationVariable;
...
public class LoginAttemptHandler {
    ...
    public LoginAttemptHandler() {
        ...
        attemptsCache = CacheBuilder.newBuilder().expireAfterWrite(userIpLockoutDuration, TimeUnit.MINUTES).build(new CacheLoader<String, Integer>() {
            @Nonnull
            @Override
